...
export default function FlyerPromptGenerator() {
  ...
  const generatePrompt = () => {
    ...
    if (secondaryList) subjectDesc += ` Style including: ${secondaryList}.`;
              ...
              {tab === "images" && (
                <div className="space-y-8 animate-in fade-in duration-500">
                  ...
                  <div className="space-y-4">
                    {[
                      { key: 'showPrimaryName', label: 'Primary Portrait' },
                      { key: 'showCelebrantSecondary', label: 'Lifestyle Action' }
                    ].map(row => (
                      <div key={row.key} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isDarkMode ? 'bg-indigo-500/5 border-indigo-500/10' : 'bg-slate-50 border-slate-200'}`}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {currentPresets.map(subject => (
                        <div key={subject} onClick={() => handleSubjectToggle(subject)} className={`flex items-center justify-between gap-3 p-4 rounded-xl border cursor-pointer transition-all ${form.secondarySubjects.includes(subject) ? "bg-indigo-600/10 border-indigo-500 shadow-lg" : isDarkMode ? "bg-slate-900/50 border-slate-800" : "bg-white border-slate-200"}`}>
